...
object formatBuilder {
          ...
          case Right(operation) =>
            operation match {
              case op if op.equals(Json.fromString("replace")) =>
                makeSolutionForReplaceOperation(pathField, newField, oldField, mapKeyWithList)(changeList, deleteList, addList)
              case op if op.equals(Json.fromString("remove")) =>
                makeSolutionForRemoveOperation(pathField, oldField, mapKeyWithList)(changeList, deleteList, addList)
              case op if op.equals(Json.fromString("add")) =>
                makeSolutionForAddOperation(pathField, newField, mapKeyWithList)(changeList, deleteList, addList)
            }
        }

      buildSolutionWithCorrectFormat(tail, mapKeyWithList)(jsonWithTheDifferences._1, jsonWithTheDifferences._2, jsonWithTheDifferences._3)
  }
  ...
  private def makeSolutionForReplaceOperation(
    ...
    val regexForKeysWithSeparator = "[a-zA-Z]+/".r
    val regexForKeys = "[a-zA-Z]+".r
    val keysWithSeparator = regexForKeysWithSeparator.findAllIn(pathField.toString()).mkString
    val keyList = regexForKeys.findAllIn(keysWithSeparator).toList

    val (keyJson, value) =
      keyList match {
        case keyList if keyList.isEmpty =>
          val regexForKeyJson = regexForKeys.findAllIn(pathField.toString()).mkString
